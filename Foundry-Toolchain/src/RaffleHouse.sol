...
contract RaffleHouse is ReentrancyGuardTransient {
        ...
        public
    {
        ...
        if (raffleEnd - raffleStart < MIN_DURATION) revert InsufficientRaffleDuration();
    ...
    function buyTicket(uint256 raffleId) public payable nonReentrant {
        ...
        if (block.timestamp < raffles[raffleId].raffleStart) revert RaffleNotStarted();
        if (block.timestamp >= raffles[raffleId].raffleEnd) revert RaffleEnded();
        if (msg.value != raffles[raffleId].ticketPrice) revert InvalidTicketPrice();
    ...
    function chooseWinner(uint256 raffleId) public nonReentrant {
        if (raffleId >= raffleCount) revert RaffleDoesNotExist();
        if (block.timestamp < raffles[raffleId].raffleEnd) revert RaffleNotEnded();
        if (raffles[raffleId].winningTicketIndex != NO_WINNER) revert WinnerAlreadyChosen();

        uint256 totalTickets = raffles[raffleId].ticketsContract.totalSupply();

        uint256 winningTicketIndex = uint256(
            keccak256(abi.encodePacked(block.timestamp, block.prevrandao, totalTickets))
        ) % totalTickets;
    ...
    function claimPrize(uint256 raffleId) public nonReentrant {
        if (raffleId >= raffleCount) revert RaffleDoesNotExist();
        if (block.timestamp < raffles[raffleId].raffleEnd) revert RaffleNotEnded();
        if (raffles[raffleId].winningTicketIndex == NO_WINNER) revert WinnerNotChosen();
        ...
        raffles[raffleId].ticketsContract.transferFrom(
            msg.sender, address(this), raffles[raffleId].winningTicketIndex
        );
